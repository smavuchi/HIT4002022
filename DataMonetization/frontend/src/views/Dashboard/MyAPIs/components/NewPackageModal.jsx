...
export default function NewAPIPackageModal({open, setOpen, onFinish, apiId, ...props}) {
  ...
  const onCreate = () => {
    ...
    axios.post(`${api}/client-api/${apiId}/packages`, { name, pricing, description, requests }).then(({data}) => {
      setLoading(false);
      if (data.status != 200 && data.status != 201) {
        ...
        onFinish();
        toast({
          title: 'API package created',
          description: "We've created your API package for you.",
          duration: 3000,
          isClosable: true,
        })
        setName(); setPricing(0); setDescription(); setRequests(10);
        onClose();
      }
    }).catch(e => {
      setLoading(false);
  ...
  return (
    <>
      <Modal onClose={onClose} isOpen={open} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>New API Package</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
