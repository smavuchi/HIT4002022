...
export default function DatasetColumns({dataset, apiId, resourceId, fetch}) {
  ...
  React.useEffect(() => {
  	setColumns(s => ({
  		keep: [],
  		discard: Object.keys(dataset.column_types)
  	}))
  }, [dataset]);
  ...
  const save = () => {
    ...
    axios.post(`${api}/client-api/${apiId}/resources/${resourceId}/dataset/resolve/columns`, data).then(({data}) => {
      setSaving(false);
  ...
  return (
      ...
      >
      	<div>
      	  <p style={{ fontWeight: "bold", padding: "10px", border: "1px solid #ddd", marginBottom: "10px" }}>Columns to discard</p>
      	  {
      	...
      	<div>
      	  <p style={{ fontWeight: "bold", padding: "10px", border: "1px solid #ddd", marginBottom: "10px" }}>Columns to keep</p>
      	  {
