...
from app.services.api_dataset.insert_median_values_where_missing import insert_median_values_where_missing as _insert_median_values_where_missing

route = router.routes["resolve missing data by inserting medians"]

@blueprint.route(route.url, methods=route.methods)
@middleware(*route.middleware, user_roles=route.roles, params={"src": "json"})
def insert_median_values_where_missing(api_id, resource_id, **kwargs):
  ...
  dataset = _insert_median_values_where_missing(dataset)
