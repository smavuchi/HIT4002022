...
from app.services.api_dataset.resolve_inconsistent_data import resolve_inconsistent_data as _resolve_inconsistent_data

route = router.routes["resolve inconsistent data"]

@blueprint.route(route.url, methods=route.methods)
@middleware(*route.middleware, user_roles=route.roles, params={"src": "query"})
def resolve_inconsistent_data(api_id, resource_id, **kwargs):
  ...
  _resolve_inconsistent_data(dataset)
