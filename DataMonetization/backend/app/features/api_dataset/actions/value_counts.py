...
from app.services.api_dataset.get_value_counts import get_value_counts_for_column
from app.services.api_dataset.get_value_counts import get_value_counts_for_columns

route = router.routes["retrieve value counts for columns"]
...
@middleware(*route.middleware, user_roles=route.roles, params={"src": "query", "defns": parameters})
def value_counts_for_columns(api_id, resource_id, column, **kwargs):
  ...
  if column:
    data = get_value_counts_for_column(dataset, column)
  else:
    data = get_value_counts_for_columns(dataset)
