...
from app.services.api_dataset.get_percentage_missing_values_by_column import get_percentage_missing_values_by_column

route = router.routes["retrieve missing values by column"]

@blueprint.route(route.url, methods=route.methods)
@middleware(*route.middleware, user_roles=route.roles, params={"src": "query"})
def missing_values_by_column(api_id, resource_id, **kwargs):
  ...
  data = get_percentage_missing_values_by_column(dataset)
