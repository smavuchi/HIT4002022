...
from app.services.api_dataset.get_missing_values_by_row import get_missing_values_by_row

route = router.routes["retrieve missing values by row"]

@blueprint.route(route.url, methods=route.methods)
@middleware(*route.middleware, user_roles=route.roles, params={"src": "query"})
def missing_values_by_row(api_id, resource_id, **kwargs):
  ...
  data = get_missing_values_by_row(dataset)
