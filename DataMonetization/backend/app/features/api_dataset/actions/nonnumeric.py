...
from app.services.api_dataset.get_nonnumeric_columns import get_nonnumeric_columns

route = router.routes["get nonnumeric dataset columns"]

@blueprint.route(route.url, methods=route.methods)
@middleware(*route.middleware, user_roles=route.roles, params={"src": "query"})
def show_nonnumeric_dataset_columns(api_id, resource_id, **kwargs):
  ...
  columns = get_nonnumeric_columns(dataset)
  return action_response({"data": columns})