...
from app.services.api_dataset.insert_frequent_values_where_missing import insert_frequent_values_where_missing as _insert_frequent_values_where_missing

route = router.routes["resolve missing data by inserting frequent values"]

@blueprint.route(route.url, methods=route.methods)
@middleware(*route.middleware, user_roles=route.roles, params={"src": "json"})
def insert_frequent_values_where_missing(api_id, resource_id, **kwargs):
  ...
  dataset = _insert_frequent_values_where_missing(dataset)
