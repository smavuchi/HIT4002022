...
from app.services.api_dataset.get_numeric_columns import get_numeric_columns

route = router.routes["get numeric dataset columns"]

@blueprint.route(route.url, methods=route.methods)
@middleware(*route.middleware, user_roles=route.roles, params={"src": "query"})
def show_numeric_dataset_columns(api_id, resource_id, **kwargs):
  ...
  columns = get_numeric_columns(dataset)
