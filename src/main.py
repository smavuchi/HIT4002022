...
st.title('Sasha Text Summarization')

text = st.text_area('Provide the text to Sasha summarizer:')

prepro_choice = ['Stemming', 'Lemmatization']
...
if st.button('Check  The Process'):
    prepro_review(text)

model = st.selectbox('Select the summarization model', ('TextRank', 'LexRank'))
